...
def cal_overwork_inten(year, radius,weight=False,):
    with h5py.File(f"./result/annual_overwork/deep/{year}_intensity.h5", 'r') as f:
        overwork = cp.array(f['data']['intensity'][:], dtype=cp.uint32)
    ...
    firmdummy=firmdummy[x_axis,y_axis]
    overwork = cp.where(firmdummy == 0, 655350, overwork)
    workdayntl=workdayntl[x_axis,y_axis]
    print(overwork)
    ...
    if not os.path.exists(f'./result/variables/deep/{radius}x{radius}_winsor/有企业的栅格的加班情况/不去掉居民区+加班绝对强度'):
        os.makedirs(f'./result/variables/deep/{radius}x{radius}_winsor/有企业的栅格的加班情况/不去掉居民区+加班绝对强度')
    if not os.path.exists(f'./result/variables/deep/{radius}x{radius}_winsor/有企业的栅格的加班情况/不去掉居民区+加班相对强度'):
        os.makedirs(f'./result/variables/deep/{radius}x{radius}_winsor/有企业的栅格的加班情况/不去掉居民区+加班相对强度')
    with h5py.File(f'./result/variables/deep/{radius}x{radius}_winsor/有企业的栅格的加班情况/不去掉居民区+加班绝对强度/{year}intensity.h5', 'w') as f:
        f.create_group('data')
        f['data'].create_dataset(name=str(year), data=overwork.get(),compression="gzip")
        f['data'].create_dataset(name='nan', data=655350)
        f['data'].create_dataset(name='x', data=x_axis)
        f['data'].create_dataset(name='y', data=y_axis)
        f['data'].create_dataset(name='description',
                                 data="使用计算出的加班强度栅格，未经过工作日平均灯光标准化，将没有企业的栅格中的加班情况赋值为空")

    with h5py.File(f'./result/variables/deep/{radius}x{radius}_winsor/有企业的栅格的加班情况/不去掉居民区+加班相对强度/R_{year}intensity.h5', 'w') as f:
        f.create_group('data')
        f['data'].create_dataset(name=str(year), data=roverwork.get(),compression="gzip")
        f['data'].create_dataset(name='nan', data=655350)
        f['data'].create_dataset(name='x', data=x_axis)
        f['data'].create_dataset(name='y', data=y_axis)
        f['data'].create_dataset(name='description',
    ...
    if weight:
        ...
        with h5py.File(f'./result/variables/deep/{radius}x{radius}_winsor/有企业的栅格的加班情况/不去掉居民区+加班绝对强度/numWeighted_{year}intensity.h5', 'w') as f:
            f.create_group('data')
        ...
        with h5py.File(f'./result/variables/deep/{radius}x{radius}_winsor/有企业的栅格的加班情况/不去掉居民区+加班相对强度/R_numWeighted_{year}intensity.h5', 'w') as f:
            f.create_group('data')
            f['data'].create_dataset(name=str(year), data=roverwork2.get(),compression="gzip")
            f['data'].create_dataset(name='x', data=x_axis)
            f['data'].create_dataset(name='y', data=y_axis)
            f['data'].create_dataset(name='nan', data=655350)
            f['data'].create_dataset(name='description',
...
def cal_overwork_ratio_withoutResident(year, x,y,radius,weight=False):
    ...
    roverwork = cp.where(workdayntl == cp.nan, 655350, roverwork)
    roverwork = cp.where(workdayntl == 0, 655350, roverwork)
    CN_overwork=655350*cp.ones((12000,16800),dtype=cp.uint32)
    CN_roverwork=655350*cp.ones((12000,16800),dtype=cp.uint32)
    CN_overwork[x_axis,y_axis]=overwork
    CN_roverwork[x_axis, y_axis] = roverwork
    CN_overwork[x,y]=655350
    CN_roverwork[x,y]=655350
    overwork=CN_overwork[x_axis,y_axis]
    roverwork = CN_roverwork[x_axis, y_axis]
    if not os.path.exists(f'./result/variables/deep/{radius}x{radius}_winsor/有企业的栅格的加班情况/去掉居民区+加班绝对强度'):
        os.makedirs(f'./result/variables/deep/{radius}x{radius}_winsor/有企业的栅格的加班情况/去掉居民区+加班绝对强度')
    if not os.path.exists(f'./result/variables/deep/{radius}x{radius}_winsor/有企业的栅格的加班情况/去掉居民区+加班相对强度'):
        os.makedirs(f'./result/variables/deep/{radius}x{radius}_winsor/有企业的栅格的加班情况/去掉居民区+加班相对强度')
    with h5py.File(f'./result/variables/deep/{radius}x{radius}_winsor/有企业的栅格的加班情况/去掉居民区+加班绝对强度/{year}intensity.h5', 'w') as f:
        f.create_group('data')
        f['data'].create_dataset(name=str(year), data=overwork.get(),compression="gzip")
        f['data'].create_dataset(name='nan', data=655350)
        f['data'].create_dataset(name='x', data=x_axis)
    ...
    with h5py.File(f'./result/variables/deep/{radius}x{radius}_winsor/有企业的栅格的加班情况/去掉居民区+加班相对强度/R_{year}intensity.h5',
                   'w') as f:
        f.create_group('data')
        f['data'].create_dataset(name=str(year), data=roverwork.get(),compression="gzip")
        f['data'].create_dataset(name='nan', data=655350)
    ...
    if weight:
        ...
        roverwork2 = cp.where(roverwork == 655350, 655350, roverwork2)
        with h5py.File(
                f'./result/variables/deep/{radius}x{radius}_winsor/有企业的栅格的加班情况/去掉居民区+加班绝对强度/numWeighted_{year}intensity.h5',
                'w') as f:
            f.create_group('data')
            f['data'].create_dataset(name=str(year), data=overwork2.get(),compression="gzip")
            f['data'].create_dataset(name='nan', data=655350)
        ...
        with h5py.File(
                f'./result/variables/deep/{radius}x{radius}_winsor/有企业的栅格的加班情况/去掉居民区+加班相对强度/R_numWeighted_{year}intensity.h5',
                'w') as f:
            f.create_group('data')
            f['data'].create_dataset(name=str(year), data=roverwork2.get(),compression="gzip")
            f['data'].create_dataset(name='nan', data=655350)
...
if __name__ == "__main__":
    ...
    for year in range(2012, 2021):
        # 保留有企业地区的加班天数占比
        # cal_overwork_inten(year,radius,weight=True)
        cal_overwork_ratio_withoutResident(year,x,y,radius,weight=True)
        # 计算栅格层面的相关系数
