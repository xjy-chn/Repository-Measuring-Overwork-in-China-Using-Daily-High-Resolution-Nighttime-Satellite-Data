...
def cal_overwork_ratio(year, weight=False, ):
    ...
    if not os.path.exists(f'./result/variables/有企业的栅格的加班情况'):
        os.makedirs(f'./result/variables/有企业的栅格的加班情况')
    with h5py.File(f'./result/variables/有企业的栅格的加班情况/{year}dummy.h5', 'w') as f:
        f.create_group('data')
    ...
    if weight:
        ...
        with h5py.File(f'./result/variables/有企业的栅格的加班情况/numWeighted_{year}dummy.h5', 'w') as f:
            f.create_group('data')
...
def cal_overwork_ratio_withoutResident(year, x, y, weight=False):
    ...
    if weight:
        ...
        overwork2 = cp.where(overwork == 101, 101, overwork2)
        with h5py.File(
                f'./result/variables/有企业的栅格的加班情况/去掉居民区+加班天数占比/DR_numWeighted_{year}dummy.h5',
                'w') as f:
            f.create_group('data')
            f['data'].create_dataset(name=str(year), data=overwork2.get())
            f['data'].create_dataset(name='nan', data=101)
