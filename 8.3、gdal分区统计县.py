...
if __name__=="__main__":
    ...
    #拼合日度加班情况
        ...
        for day in days:
            ...
            #                                   value_to_nodata=2)
            arcpy.sa.ZonalStatisticsAsTable(r"F:\日度夜间灯光\原始数据\2019年中国各级行政区划\v84\县_WGCS1984.shp",
                                            "PAC", arcpy.NumPyArrayToRaster(dummy, arcpy.Point(70, 10),
                                                                               x_cell_size=0.004166666666666667,
                                                                               y_cell_size=0.004166666666666667,
                                                                               value_to_nodata=2),
                                            rf"F:\popLight\日度灯光分区统计\daily_zontal.gdb\zco{year}{day}", "DATA",
                                            "ALL", "CURRENT_SLICE", 90, "AUTO_DETECT", "ARITHMETIC", 360)
            print(f"第{year}年第{day}天加班按县分区统计已输出完毕")
        # time.sleep(1000)
