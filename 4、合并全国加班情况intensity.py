...
if __name__ == "__main__":
    for year in range(2012,2021):
        intensity = 655350 * np.ones((5 * 2400, 7 * 2400), dtype=np.uint32)
        for v in range(5):
            for h in range(7):
        ...
        with h5py.File(f'./result/annual_overwork/national/intensity/intensity{year}.h5', 'w') as f:
            f.create_group('data')
            f['data'].create_dataset(name=str(year), data=intensity)
        ...
        # dummy = 101*np.ones((5 * 2400, 7 * 2400), dtype=np.uint8)
        # # print(dummy)
        # for v in range(5):
        #     for h in range(7):
        ...
        # with h5py.File(f'./result/annual_overwork/national/dummy/dummy{year}.h5','w') as f:
        #     f.create_group('data')
        #     f['data'].create_dataset(name=str(year),data=dummy)
