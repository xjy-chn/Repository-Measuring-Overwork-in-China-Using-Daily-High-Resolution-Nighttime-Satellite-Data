...
if __name__=="__main__":
    ...
    # #导入企业栅格
        ...
        if not os.path.exists(f'./result/日度分区统计/企业加权暂存/{radius}x{radius}/{year}'):
            os.makedirs(f'./result/日度分区统计/企业加权暂存/{radius}x{radius}/{year}')
    #拼合日度加班情况
        ...
        for day in days:
            ...
            with h5py.File(f'./result/日度分区统计/企业加权暂存/{radius}x{radius}/{year}/{day}.h5 ','w') as f:
                dataset=f.create_group('data')
                f['data'].create_dataset(name=day, data=dummy2.get(),compression="gzip")
