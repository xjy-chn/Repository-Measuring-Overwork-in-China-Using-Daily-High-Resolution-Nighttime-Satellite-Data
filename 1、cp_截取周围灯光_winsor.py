...
def save_move(data,firm, x,y,year, day, direction,description, r,bound=True):
    # print(filename)
    if bound:
        if not os.path.exists(f'./result/deep/surrounding_pic/{r}x{r}_winsored/{year}/{direction}'):
            os.makedirs(f'./result/deep/surrounding_pic/{r}x{r}_winsored/{year}/{direction}')
        with h5py.File(f'./result/deep/surrounding_pic/{r}x{r}_winsored/{year}/{direction}/{day}.h5', "w") as f:
            f.create_group('imformation')
            f.create_group('data')
            f['data'].create_dataset(name=day, data=data,compression="gzip")
...
def save_move_self(data,firm, x,y,year, day,description, r,bound=True):
    # print(filename)
    if bound:
        if not os.path.exists(f'./result/deep/surrounding_pic/{r}x{r}_winsored/{year}'):
            os.makedirs(f'./result/deep/surrounding_pic/{r}x{r}_winsored/{year}')
...
def cal_surrounding_median_dummy(left, right,r):
    ...
    for key, value in daily_files.items():
        ...
        if not os.path.isfile(f'./result/deep/surrounding_pic/{year}/{day}.h5'):
            ...
            view2=view[x,y,:,:]
            view_firm2=view_firm[x,y,:,:]
            view=None
            x=cp.array(x,dtype=cp.uint16)
            y=cp.array(y,dtype=cp.uint16)
            print(x.shape,y.shape)
            # time.sleep(100)
            save_move_self(data=view2.get().astype(np.uint16),firm=view_firm2.get(),x=x.get(),y=y.get(), year=year, day=day,
