...
if __name__ == "__main__":
    ...
    for year in range(2012, 2021):
        ...
        with h5py.File(days_inten_fp[0],'r') as f:
            print(f['data'].keys())
            print(f['data']['intensity'][:].shape[0])
            data = 65535 * cp.ones((len(days_inten_fp),f['data']['intensity'][:].shape[0]), dtype=cp.uint16)
            x=list(f['data']['x'][:])
            y=list(f['data']['y'][:])
        #导入深度学习算法得到的结果
        for i in range(len(days_inten_fp)):
            if os.path.isfile(days_inten_fp[i]):
                with h5py.File(days_inten_fp[i],'r') as f:
                    data[i,:]=cp.array(f['data']['intensity'][:])
        ...
        mask = np.isin(data.get(), values_to_exclude)
        ...
        # time.sleep(100)
        intensity = np.where(no_missing.get() == 0, 655350, np.array(intensity, dtype=np.uint32))
        intensity = np.where(CN_works_no_missing.get() < 70, 655350, intensity)
        intensity = np.where(CN_holidays_no_missing.get() < 5, 655350, intensity)

        with h5py.File(fr'./result/annual_overwork/deep/{year}_intensity.h5','w') as f:
            f.create_group('data')
            f['data'].create_dataset(name='x',data=x,compression='gzip')
            f['data'].create_dataset(name='y', data=y, compression='gzip')
            f['data'].create_dataset(name='intensity', data=intensity, compression='gzip')
        pass
