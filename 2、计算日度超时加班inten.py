...
def save_overwork(data, date, year, description, block, type,r,winsor=True):
    ...
    if winsor:
        ...
        with h5py.File(f'./result/daily_overwork/{r}x{r}_winsor/{year}/{block}/{type}' + '/' + filename + '.h5', "w") as f:
            f.create_group('imformation')
            f.create_group('data')
    ...
    for year in range(2012,2021):
        ...
        for i in range(len(all)):
            ...
            if key in daily_files.keys():
                ...
                if winsor:
                    surrounding_fp = f'./result/surrounding_median/national/{radius}x{radius}_winsored/{year}/{date}.h5'
                    #第三个条件，周围有效栅格数量
                    valid_fp=f'./result/surrounding_valid/{radius}x{radius}_winsor/{year}/{date}.h5'
                else:
                    surrounding_fp = f'./result/surrounding_median/national/{radius}x{radius}/{year}/{date}.h5'
                    # 第三个条件，周围有效栅格数量
                    valid_fp = f'./result/surrounding_valid/{radius}x{radius}/{year}/{date}.h5'
                ...
                # print(overwork_intensity[9600:12000,14400:16800])
                # print(overwork_intensity.shape)
                # print(sur_valid[9600:12000,14400:16800])
                # time.sleep(100)
                #周边栅格中位数缺失
                ...
                # time.sleep(1000)
                # print(overwork_dummy[9600:12000,14400:16800])
                # print(overwork_intensity.shape)
                # print(sur_valid[9600:12000,14400:16800])
                # time.sleep(100)
                # 检查缺失值是否复原
