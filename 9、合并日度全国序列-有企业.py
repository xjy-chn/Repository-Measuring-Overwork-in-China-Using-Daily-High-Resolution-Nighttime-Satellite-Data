...
if __name__=="__main__":
    for year in range(2012,2021):
        data = pd.DataFrame()
        annual_files=sorted(os.listdir(f'./result/日度分区统计/全国有企业/{year}'))
        abs_Afiles_fp=sorted([f'./result/日度分区统计/全国有企业/{year}'+'/'+file for file in annual_files])
        print(annual_files)
        print(abs_Afiles_fp)
        ...
        data.to_excel(f'./result/日度分区统计/全国有企业/{year}年度汇总.xlsx',index=False)
            # da
